...
The patient indicated whether they took their medication today: ${medicationTaken ? 'YES' : 'NO'}.`
      ...
      if (
        ...
        ((lowerText.includes('fever') || lowerText.includes('bukhar')) && !lowerText.includes('low') && !lowerText.includes('mild'))
      ) {
        risk = 'HIGH';
    ...
    if (req.app.get('io')) {
      ...
      const notif = new Notification({
        ...
        userModel: 'Doctor',
        type: 'Record',
        content: `${patient.name} recorded a new AI triage log (${risk} risk).`
      });
      await notif.save();
      io.to(doctorId).emit('new-notification', notif);
    }

    // ✅ 🚨 REAL SMS LOGIC (ADDED)
    if (risk === 'HIGH') {
      try {
